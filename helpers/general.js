...
var putSegmentsInPromise = function (segments, childWorkerPath, data) {
    if (data === void 0) { data = {}; }
    return segments.map(function (segment) { return new Promise(function (resolve, reject) {
        // transfer data to each workers/threads
        var worker = new worker_threads_1.Worker(childWorkerPath, {
            workerData: __assign({ segment: segment }, data)
        });
...
var isCurrentUser = function (_id, user) { return _id.toString() === user._id.toString(); };
var general = {
    isCurrentUser: isCurrentUser,
    jsonParser: jsonParser,
    putSegmentsInPromise: putSegmentsInPromise
};
